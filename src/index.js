...
async function getGoogleAccessToken(env, scope) {
  ...
  const assertion = await createServiceAccountJwt(serviceAccount, scope);
  const response = await fetch(GOOGLE_TOKEN_URL, {
    method: 'POST',
  ...
  if (!response.ok) {
    throw new Error(`Google OAuth token error (${response.status}): ${JSON.stringify(body)}`);
  }

  cachedAccessToken = { value: body.access_token, scope: cacheKey };
...
async function getActiveDevices(env, uid) {
  const projectId = parseServiceAccount(env).project_id;
  const parent = `${firestoreBase(projectId)}/users/${encodeURIComponent(uid)}`;
  const token = await getGoogleAccessToken(env, FIRESTORE_SCOPE);
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Firestore devices query error (${response.status}): ${text}`);
  }
  ...
  return rows
    ...
    .filter((doc) => cleanString(doc.token));
}

async function updateFirestoreDocuments(env, writes) {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Firestore commit error (${response.status}): ${text}`);
  }
}

function buildData(notification) {
  const data = {
    notificationId: cleanString(notification.id),
    tournamentId: cleanString(notification.tournamentId),
    type: cleanString(notification.type),
    target: cleanString(notification.target),
    matchId: cleanString(notification.matchId),
    round: cleanString(notification.round),
    title: cleanString(notification.title),
    body: cleanString(notification.body),
    imageUrl: cleanString(notification.imageUrl),
  };
  ...
  if (extra && typeof extra === 'object' && !Array.isArray(extra)) {
    for (const [key, value] of Object.entries(extra)) {
      ...
      data[`extra_${key}`] = typeof value === 'string' ? value : JSON.stringify(value);
    }
  }
...
async function sendFcmMessage(env, token, notification) {
  const projectId = parseServiceAccount(env).project_id;
  const accessToken = await getGoogleAccessToken(env, FIREBASE_MESSAGING_SCOPE);

  const message = {
    token,
    notification: {
      title: cleanString(notification.title) || 'Ptola',
      body: cleanString(notification.body),
    },
    ...
    android: {
      priority: 'HIGH',
      notification: {
        channel_id: 'ptola_notifications',
        sound: 'default',
        default_sound: true,
        default_vibrate_timings: true,
        notification_count: 1,
...
async function sendToRecipient(env, notification) {
  const recipientUid = cleanString(notification.recipientUid);
  if (!recipientUid) throw new Error('recipientUid is required.');

  const devices = await getActiveDevices(env, recipientUid);
  const uniqueDevices = [...new Map(devices.map((device) => [device.token, device])).values()];
  ...
  const results = await Promise.all(
    uniqueDevices.map(async (device) => {
      try {
        const result = await sendFcmMessage(env, device.token, notification);
        return { device, result };
      } catch (error) {
        return { device, result: { ok: false, status: 500, body: { error: error.message } } };
      }
    }),
  );
  ...
  const invalidDevices = results.filter(({ result }) => isInvalidFcmToken(result));

  if (invalidDevices.length) {
    const writes = invalidDevices.map(({ device }) => ({
      update: {
        name: device.name,
        fields: {
      ...
      updateTransforms: [
        { fieldPath: 'disabledAt', setToServerValue: 'REQUEST_TIME' },
      ],
    }));

    await updateFirestoreDocuments(env, writes);
  }

  return {
    successCount,
    failureCount,
    invalidDeviceIds: invalidDevices.map(({ device }) => device.name),
  };
}

async function authenticateAdmin(env, request) {
  ...
  if (!idToken) {
    const error = new Error('Empty Firebase ID token.');
    error.statusCode = 401;
    throw error;
  }

  const decoded = await verifyFirebaseIdToken(env, idToken);
  const user = await getUser(env, decoded.sub);
...
async function handleSend(env, request) {
  ...
  if (!notification || typeof notification !== 'object' || Array.isArray(notification)) {
    return json({ ok: false, error: 'notification object is required.' }, 400);
  }

  const notificationId = cleanString(notification.id);
  if (!notificationId) {
    return json({ ok: false, error: 'notification.id is required.' }, 400);
  }

  const result = await sendToRecipient(env, notification);
  const projectId = parseServiceAccount(env).project_id;
  const notificationPath = `${firestoreBase(projectId)}/notifications/${encodeURIComponent(notificationId)}`;

  const notificationFields = {
    pushStatus: firestoreValue(result.successCount > 0 ? 'sent' : 'failed'),
    pushSuccessCount: firestoreValue(result.successCount),
    pushFailureCount: firestoreValue(result.failureCount),
  };
  ...
  const writes = [{
    ...
    updateMask: { fieldPaths: Object.keys(notificationFields) },
    updateTransforms: [
      { fieldPath: 'pushSentAt', setToServerValue: 'REQUEST_TIME' },
    ],
  }];

  await updateFirestoreDocuments(env, writes);

  return json({ ok: true, notificationId, ...result });
}
  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/health') {
        return json({ ok: true, service: 'ptola-notification-backend' });
      }

      if (request.method === 'POST' && url.pathname === '/v1/notifications/send') {
        return await handleSend(env, request);
      }
